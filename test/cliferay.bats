...
@test "cliferay todos tui" {
    ...
    # Without Claude installed the board says so instead of clearing the screen.
    run bash -c "{ printf 'c'; sleep 0.4; printf 'q'; } | CLIFERAY_TODOS_TUI=1 cliferay todos | sed 's/\x1b\[[0-9;]*[A-Za-z]//g'"
    assert_output --partial "c claude"
    assert_output --partial "Claude is not installed"

    # c hands the selected todo to Claude, and the board is back afterwards.
    printf '#!/bin/sh\necho "$@" > %s/claude.log\n' "$TMP_DIR" > $TMP_DIR/mocks/claude
    chmod +x $TMP_DIR/mocks/claude
    printf 'jcq' | CLIFERAY_TODOS_TUI=1 cliferay todos > /dev/null
    rm -f $TMP_DIR/mocks/claude
    run cat $TMP_DIR/claude.log
    assert_output "--dangerously-skip-permissions /cliferay:todo work on Third one"
