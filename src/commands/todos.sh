...
draw_list() {
    ...
    if [ -n "$STATUS" ]; then
        ...
        case ${TABS[$TAB]} in
            todo) hints up/down select left/right tab enter read o edit c claude shift+up/down priority t top d done f forget n new q quit ;;
...
draw_read() {
    ...
    case ${TABS[$TAB]} in
        todo) hints up/down scroll o edit c claude d done f forget q back ;;
...
# Hands the todo to Claude, which manages the board through the cliferay skill.
open_claude() {
    [ "${TABS[$TAB]}" = todo ] || return
    [ "${#ENTRIES[@]}" -gt 0 ] || return
    if ! command -v claude > /dev/null 2>&1; then
        STATUS="Claude is not installed"
        return
    fi
    title_of todo "${ENTRIES[${SEL[$TAB]}]}"
    screen_off
    # What the cc alias runs, since aliases do not reach a script.
    claude --dangerously-skip-permissions "/cliferay:todo work on $TITLE"
    screen_on
    # Claude may have finished or reshuffled things while it had the terminal.
    TITLES=()
    reload
}
