...
send_completions() {
  ...
  echo $''
  echo $'    \'curl\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_cliferay_completions_filter "--help -h ai batch-import new-api new-instance new-object new-site-object")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'sql\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_cliferay_completions_filter "--help -h")" -- "$cur")'
